...
; The variable AntonymPos (start) and AntonymStop (end) mark where antonyms start and stop in a page.
     ...
     AntonymPos := RegExMatch(SynPage,"Antonyms <span class")        
     AntonymStop := RegExMatch(SynPage,"More words related to <strong>")        
...
; The RegExMatch function  extracts each synonym based upon unique surrounding HTML code.
  ...
  SynPos := RegExMatch(SynPage,"<a href=""/browse/(.*?)"".*?>(.*?)</a></span></li>",Synonym,NewSynPos)        ; 
...
; When found between AntonymPos and AntonymStop, the word is an antonym adding a special warning icon.
	...
	If (AntonymPos != 0) and (AntonymPos < SynPos) and (SynPos < AntonymStop)    
